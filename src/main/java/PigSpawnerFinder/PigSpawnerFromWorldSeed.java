...
public class PigSpawnerFromWorldSeed {
	...
	public static void processForChunk(long worldSeed,int chunkX,int chunkZ){
		...
		for (Spawner spawner : spawners) {
			...
			if (!(spawner.direction.axis == Direction.Axis.X && spawnerOffsetZ == 9 && (spawnerOffsetX == 8 || spawnerOffsetX == 10)) && !(spawner.direction.axis == Direction.Axis.Z && spawnerOffsetX == 9 && (spawnerOffsetZ == 8 || spawnerOffsetZ == 10)))
				continue;
			...
			System.out.println("Spawner could work for : " + spawner);
			processCarverSeed(worldSeed, spawner.x, spawner.y, spawner.z, spawner.direction, spawner.length);
		}
	}
	...
	public static void findOutIfCorrect(long worldseed, Spawner spawner) {
		...
		int spawnerOffsetZ =spawner.x & 15;
...
//                    System.out.println(spawnerOffset + spawnerShift);
		...
		System.out.println("Structure: " + structureSeed + " Center: " + spawnerChunkX + " " + spawnerChunkX + " Spawner: " +spawnerPos);
...
//            System.out.println("Good central height: " + worldSeed);
		...
		System.out.println("Good nearby height: " + worldSeed + " " + spawnerPos);
	}
}
